    ...
    <section className="relative overflow-hidden border-y border-border bg-surface px-6 py-28" id="verifier">
      ...
      <div className="relative mx-auto max-w-wide">
        <div className="mx-auto max-w-narrow text-center">
          <p className="text-xl text-mint" style={{ fontFamily: 'var(--font-caveat), cursive' }}>( Proof, not promises )</p>
          ...
          <p className="mx-auto mt-7 max-w-prose text-base leading-relaxed text-muted">The verifier is designed to recompute reconciliation from the exported events rather than trusting the summary it receives.</p>
        </div>
        ...
        <div className="mt-16 overflow-hidden rounded-feature-lg bg-[radial-gradient(circle_at_50%_0%,#1b352a_0%,#0f1713_48%,#080d0a_100%)] shadow-2xl shadow-ink/20">
          <div className="grid md:grid-cols-2">
            <div className="border-b border-white/[0.07] p-8 sm:p-10 md:border-b-0 md:border-r"><div className="mb-7 flex items-center justify-between"><span className="text-xs font-medium tracking-wide text-mint">UNTOUCHED BUNDLE</span><span className="mono text-[9px] text-white/25">valid</span></div><div className="mb-7 border-y border-white/[0.07] py-4"><CheckLine label="manifest complete" state="pass" /><CheckLine label="signature valid against pinned key" state="pass" /><CheckLine label="event chain integrity" state="pass" /><CheckLine label="reconciliation matches summary" state="pass" /></div><p className="font-heading text-2xl text-white">PASS <span className="ml-2 text-sm font-normal text-white/35">all checks passed</span></p></div>
            <div className="p-8 sm:p-10"><div className="mb-7 flex items-center justify-between"><span className="text-xs font-medium tracking-wide text-danger">QUANTITY 1 → 2</span><span className="mono text-[9px] text-white/25">tampered</span></div><div className="mb-7 border-y border-white/[0.07] py-4"><CheckLine label="manifest complete" state="pass" /><CheckLine label="FILE_CHECKSUM_MISMATCH" state="fail" /><div className="ml-6 space-y-1 pb-2"><p className="mono text-[10px] text-white/25">file: events.json</p><p className="mono text-[10px] text-danger/70">observed e08f20b8c474…</p></div><CheckLine label="recomputation mismatch" state="fail" /></div><p className="font-heading text-2xl text-white">FAIL <span className="ml-2 text-sm font-normal text-white/35">integrity violation</span></p></div>
          </div>
        </div>

        <div className="mt-8 grid gap-3 md:grid-cols-3">
          {coreGuarantees.map((item, index) => <div key={item.claim} className="rounded-card border border-border bg-canvas p-5"><span className="mono text-[9px] text-mint">0{index + 1}</span><p className="mt-3 text-sm font-heading text-ink">{item.claim}</p><p className="mt-2 text-xs leading-relaxed text-muted">{item.evidence}</p></div>)}
        </div>
